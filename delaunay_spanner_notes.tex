...
From lemmas \ref{lemma:monotonic:path} and
\ref{lemma:boundary:circles}, it follows that the one-sided path is at
most $\pi/2$ times as long as the euclidean distance between the
endpoints.

% TODO: give a nice explanation of this
...
% TODO: explain how this means the shortcut path is bounded
...
\begin{proof}
  ...
  By lemmas \ref{lemma:boundary:circles} and
  \ref{lemma:convex:path:one:sided}, we know that the length of the
  shortcut path is the sum of the one-sided paths in the lower convex
  hull between $b_i$ and $b_j$ which is at most $t \cdot \pi / 2$.
